...
const api = Statika(
  coordinates(
    "http",
    process.env.STATIKA_HOST ?? "localhost",
    process.env.STATIKA_PORT ?? 7070
  )
);
const bucket = uuid();
...
beforeAll(async () => {
  try {
    bean = await api.system.issueToken(
      process.env.UPLOAD_KEY ?? "default key ",
      bucket
    );
  } catch (e) {
...
describe("Upload", () => {
  test("no directory", async () => {
    await expect(
      uploadAllFilesInFolder(api, bean, `${Math.random()}`)
    ).rejects.toThrow(/no such file or directory/);
  });

  test("empty directory", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "empty-"));
    await expect(uploadAllFilesInFolder(api, bean, tmp)).rejects.toThrow(
      /has no files in it/
    );
    fs.rmdirSync(tmp);
  });
